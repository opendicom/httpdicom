...
#pragma mark create mwlitem
   ...
   else if ([pacs[@"mwl"]isEqualToString:@"rest"])
   {
    ...
    else if ([pacs[@"mwl"]isEqualToString:@"sql"])
    {
...
#pragma mark qido of the stow, in order to actualize metadata in pacs
    NSMutableURLRequest *GETqidostudiesRequest=
    [NSMutableURLRequest
     GETqidostudies:[dcm4cheelocaluri stringByAppendingPathComponent:@"rs/studies"]
     studyUID:studyUID
     timeout:60
     ];
    LOG_VERBOSE(@"[mwlitem] <CDA> GET -> %@\r\n ", GETqidostudiesRequest.URL.absoluteString);
    NSHTTPURLResponse *GETqidostudiesResponse=nil;
    NSError *qidoinstanceerror=nil;

    NSData *GETqidostudiesResponseData=[NSURLConnection sendSynchronousRequest:GETqidostudiesRequest returningResponse:&GETqidostudiesResponse error:&qidoinstanceerror];
    if (GETqidostudiesResponse.statusCode!=200)
    {
        NSString *GETqidostudiesResponseString=[[NSString alloc]initWithData:GETqidostudiesResponseData encoding:NSUTF8StringEncoding];
        LOG_ERROR(@"[mwlitem] <CDA> GET <- %ld %@", GETqidostudiesResponse.statusCode, GETqidostudiesResponseString);
