 ...
 returnAttributes:(BOOL)returnAttributes
{
    if (!an || ![an length])
    {
        LOG_WARNING(@"[NSURLSessionDataTask+DRS] no accession number");
        return nil;
    }
    ...
    if (issuerLocal) request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/rs/studies?AccessionNumber=%@&00080051.00400031=%@&includefield=00100021",pacs[@"dcm4cheelocaluri"],an,issuerLocal]]
                                                  cachePolicy:cachepolicy
                                              timeoutInterval:timeoutinterval];
    if (issuerUniversal && issuerType) request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/rs/studies?AccessionNumber=%@&00080051.00400032=%@&00080051.00400033=%@&includefield=00100021",pacs[@"dcm4cheelocaluri"],an,issuerUniversal,issuerType]]
                                                       cachePolicy:cachepolicy
                                                   timeoutInterval:timeoutinterval];
    else  request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/rs/studies?AccessionNumber=%@&includefield=00100021",pacs[@"dcm4cheelocaluri"],an]]
                                            cachePolicy:cachepolicy
                                        timeoutInterval:timeoutinterval];
    ...
    else
    {
        ...
        if (response.statusCode==200)
        {
            ...
            if (error)
            {
                LOG_WARNING(@"[NSURLSessionDataTask+DRS] GETAccessionNumber badly formed json answer: %@", [error description]);
                return nil;
            }
            if ([arrayOfDicts count]==0) return nil;
            if ([arrayOfDicts count]!=1)
            {
                LOG_WARNING(@"[NSURLSessionDataTask+DRS] GETAccessionNumber more than one study identified by an:%@ issuerLocal:%@ issuerUniversal:%@ issuerType:%@", an, issuerLocal, issuerUniversal, issuerType);
                return nil;
            }
            return arrayOfDicts[0];
        }
        LOG_WARNING(@"[NSURLSessionDataTask+DRS] GETAccessionNumber %ld",response.statusCode);
        if (error) LOG_ERROR(@"[NSURLSessionDataTask+DRS] GETAccessionNumber error:\r\n%@",[error description]);
    }
    return nil;
}
       ...
       returnAttributes:(BOOL)returnAttributes
{
    ...
    {
        LOG_WARNING(@"[NSURLSessionDataTask+DRS] no studyUID");
        return nil;
    }
