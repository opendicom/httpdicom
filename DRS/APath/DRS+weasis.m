...
-(void)addWeasisStudiesHandler
{
 ...
 [self addHandler:@"GET" regex:mwstudiesRegex processBlock:
 ...
 NSDictionary *entityDict=DRS.pacs[q[@"custodianOID"]];
 ...
 if (an)sqlString=[NSString stringWithFormat:sqlobjectmodel[@"manifestWeasisStudyAccessionNumber"],entityDict[@"sqlprolog"],an];
 else
 {
 NSString *euid=request.query[@"StudyInstanceUID"];
 if (euid)sqlString=[NSString stringWithFormat:sqlobjectmodel[@"manifestWeasisStudyStudyInstanceUID"],entityDict[@"sqlprolog"],euid];
...
-(void)addWeasisSeriesHandler
{
 ...
 [self addHandler:@"GET" regex:mwseriesRegex processBlock:
 ^(RSRequest* request, RSCompletionBlock completionBlock)
 {completionBlock(^RSResponse* (RSRequest* request)
 {
 LOG_DEBUG(@"client: %@",request.remoteAddressString);
 ...
 NSString *p=requestURL.path;
 NSString *euid=pComponents[4];
 NSString *suid=pComponents[6];
 ...
 NSString *sqlString=[NSString stringWithFormat:sqlobjectmodel[@"manifestWeasisSeriesStudyInstanceUIDSeriesInstanceUID"],entityDict[@"sqlprolog"],euid,suid];
 ...
 int studiesResult=task(@"/bin/bash",
 @[@"-s"],
 [[NSString stringWithFormat:sqlobjectmodel[@"manifestWeasisStudyStudyInstanceUID"],entityDict[@"sqlprolog"],euid] dataUsingEncoding:NSUTF8StringEncoding],
 studiesData
 );
 if (charset==5) //latin1
 {
