...
[self addHandler:@"GET" regex:iheiidRegex processBlock:
 ^(RSRequest* request, RSCompletionBlock completionBlock){completionBlock(^RSResponse* (RSRequest* request)
{
  ...
  if (!q[@"custodianOID"]) return [RSDataResponse responseWithText:[NSString stringWithFormat:@"missing custodianOID param in %@%@?%@",b,p,requestURL.query]];
   ...
   NSString *additionalParameters=(DRS.pacs[q[@"custodianOID"]])[@"wadoadditionalparameters"];
   if (!additionalParameters)additionalParameters=@"";
   
   NSString *custodianglobaluri;
   if (![(DRS.pacs[q[@"custodianOID"]])[@"custodianglobaluri"] length]) custodianglobaluri=[NSString stringWithFormat:@"http://localhost:%lld",DRS.drsport];
   else custodianglobaluri=(DRS.pacs[q[@"custodianOID"]])[@"custodianglobaluri"];
   
   NSString *qidoBaseURI;
   if ([(DRS.pacs[q[@"custodianOID"]])[@"qido"] length])
      qidoBaseURI=(DRS.pacs[q[@"custodianOID"]])[@"qido"];
   else qidoBaseURI=[NSString stringWithFormat:@"http://localhost:%lld/pacs/%@/rs",DRS.drsport,q[@"custodianOID"]];
      ...
      )
  {
      ...
      {
          if (q[@"accessionNumber"]) manifestWeasisURI=[NSString stringWithFormat:@"%@/manifest/weasis/studies?AccessionNumber=%@&custodianOID=%@",custodianglobaluri,q[@"accessionNumber"],q[@"custodianOID"]];
          else if (q[@"studyUID"]) manifestWeasisURI=[NSString stringWithFormat:@"%@/manifest/weasis/studies?StudyInstanceUID=%@&custodianOID=%@",custodianglobaluri,q[@"studyUID"],q[@"custodianOID"]];
      ...
      {
          //SERIES
          if (q[@"studyUID"] && q[@"seriesUID"]) manifestWeasisURI=[NSString stringWithFormat:@"%@/manifest/weasis/studies/%@/series/%@?custodianOID=%@",custodianglobaluri,q[@"studyUID"],q[@"seriesUID"],q[@"custodianOID"]];
      ...
      if (![custodianglobaluri isEqualToString:@"http://localhost"])
      {
  ...
  else if ([viewerType isEqualToString:@"cornerstone"])
  {
      ...
      if ([requestType isEqualToString:@"STUDY"])
      {
          if (q[@"accessionNumber"]) qidoSeriesString=[NSString stringWithFormat:@"%@/pacs/2.16.858.0.1.4.0.72769.217215590012.2/rs/series?AccessionNumber=%@",custodianglobaluri,q[@"accessionNumber"]];
          else if (q[@"studyUID"]) qidoSeriesString=[NSString stringWithFormat:@"%@/pacs/2.16.858.0.1.4.0.72769.217215590012.2/rs/series?StudyInstanceUID=%@",custodianglobaluri,q[@"studyUID"]];
          else return [RSDataResponse responseWithText:[NSString stringWithFormat:@"requestType=STUDY requires param accessionNumber or studyUID in %@%@?%@",b,p,requestURL.query]];
      }
      else
      {
          //SERIES
          if (q[@"studyUID"] && q[@"seriesUID"]) qidoSeriesString=[NSString stringWithFormat:@"%@/pacs/2.16.858.0.1.4.0.72769.217215590012.2/rs/series?StudyInstanceUID=%@&SeriesInstanceUID=%@",custodianglobaluri,q[@"studyUID"],q[@"seriesUID"]];
          else return [RSDataResponse responseWithText:[NSString stringWithFormat:@"requestType=SERIES requires params studyUID and seriesUID in %@%@?%@",b,p,requestURL.query]];
      }
      ...
      [cornerstone setObject:sd forKey:@"studyDate"];
